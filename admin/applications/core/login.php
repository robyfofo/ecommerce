...
switch(Core::$request->method) {
	case 'check':
		if (isset($_POST['submit'])) {
			...
			if ($_POST['username'] == "") {
				$App->error = 0;
				$App->messages = Config::$localStrings['Devi inserire un nome utente!'];
				} else {
			...
			if ($_POST['password'] == "") {
				$App->error = 1;
				$App->messages  = Config::$localStrings['Devi inserire la password!'];
				} else { 
			...
			if ($App->error == 0) {					
				...
				Sql::initQuery(Config::$dbTablePrefix.'users',
				array('id','name','surname','username','password','id_level','avatar','template','is_root'),
				array(),
				"username = '".$username."' AND active = 1 AND in_admin = 1");
					...
					if (Sql::getFoundRows() > 0) {
						...
						if (password_verify($password,$App->item->password)) {
							...
							die();						
								...
								Core::$resultOp->messages[] = 'bbb'.Config::$localStrings['I dati inseriti non corrispondono!'];
						}
					
					} else {
						Core::$resultOp->messages[] = Config::$localStrings['I dati inseriti non corrispondono!'];
					}		
				} else  { 
				Core::$resultOp->message = Config::$localStrings['Errore accesso db!'];
				}	
				
			} else  {
			Core::$resultOp->message = Config::$localStrings['Accesso negato!'];        		
			}	
