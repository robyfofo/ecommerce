<?php
/* location/Comuni.php v.1.0.0. 10/02/2021 */
...
$App->location_province = Sql::getRecords();

switch (Core::$request->method) {

	case 'activeComuni':
	case 'disactiveComuni':
		Sql::manageFieldActive(substr(Core::$request->method,0,-6),$App->params->tables['comuni'],$App->id,array('label'=>$localStrings['comune'],'attivata'=>$localStrings['attivato'],'disattivata'=>$localStrings['disattivato']));
	...
	case 'deleteComuni':
		if ($App->id > 0) {
			...
			$_SESSION['message'] = '0|'.ucfirst(preg_replace('/%ITEM%/',$localStrings['comune'],$localStrings['%ITEM% cancellato'])).'!';
			ToolsStrings::redirect(URL_SITE.Core::$request->action.'/listComuni');
	...
	case 'newComuni':	
		...
		Sql::initQuery($App->params->tables['nations'],array('*'),array(),'active = 1','title_'.$localStrings['user'].' ASC');
		...
		$App->pageSubTitle = preg_replace('/%ITEM%/',$localStrings['comune'],$localStrings['inserisci %ITEM%']);
	...
	case 'insertComuni':
		if ($_POST) {
			...
			if (isset($_POST['location_nations_id']) && intval($_POST['location_nations_id']) > 0) {
				...
				Sql::initQuery($App->params->tables['nations'],array('title_'.$localStrings['user']),array(intval($_POST['location_nations_id'])),'id = ? AND active = 1');
			...
			Form::parsePostByFields($App->params->fields['comuni'],$localStrings,array());
			...
			$_SESSION['message'] = '0|'.ucfirst(preg_replace('/%ITEM%/',$localStrings['comune'],$localStrings['%ITEM% inserito'])).'!';
			ToolsStrings::redirect(URL_SITE.Core::$request->action.'/listComuni');
	...
	case 'modifyComuni':	
		...
		$App->pageSubTitle = preg_replace('/%ITEM%/',$localStrings['comune'],$localStrings['modifica %ITEM%']);
	...
	case 'updateComuni':
		if ($_POST) {
			...
			$_SESSION['message'] = '0|'.ucfirst(preg_replace('/%ITEM%/',$localStrings['comune'],$localStrings['%ITEM% modificato'])).'!';
	...
	default;
		...
		if (isset($sessClause) && $sessClause != '') $whereClause .= $whereAndClause.'('.$sessClause.')';
		if (is_array($qryFieldsValuesClause) && count($qryFieldsValuesClause) > 0) {
			$fieldsValues = array_merge($fieldsValues,$qryFieldsValuesClause);
			$whereAndClause = ' AND ';	
		}

		if (isset($_POST['filters'])) {
			$_SESSION[$App->sessionName] = Applications::setFiltersSessions($_POST['filters'],$_SESSION[$App->sessionName]);
		}

		if (isset($_SESSION[$App->sessionName])) {
			list($whereClause,$whereAndClause,$fields,$fieldsValues) = Applications::getWhereClauseDBFromFiltersPost(
				$_SESSION[$App->sessionName],
				$whereClause,
				$whereAndClause,
				$fields,
				$fieldsValues,
		...
		Sql::initQuery($App->params->tables['comuni'],$fields,$fieldsValues,$whereClause);
		...
		$App->pageSubTitle = preg_replace('/%ITEMS%/',$localStrings['comuni'],$localStrings['lista %ITEMS%']);
		$App->viewMethod = 'list';		
	break;	
}
