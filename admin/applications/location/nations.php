...
switch (Core::$request->method) {
	...
	case 'disactiveNation':
		Sql::manageFieldActive(substr(Core::$request->method,0,-6),$App->params->tables['nations'],$App->id,array('label'=>$localStrings['nazione'],'attivata'=>$localStrings['attivata'],'disattivata'=>$localStrings['disattivata']));
	...
	case 'deleteNation':
		if ($App->id > 0) {
			...
			$_SESSION['message'] = '0|'.ucfirst(preg_replace('/%ITEM%/',$localStrings['nazione'],$localStrings['%ITEM% cancellata'])).'!';
			ToolsStrings::redirect(URL_SITE.Core::$request->action.'/listNation');
	...
	case 'newNation':
		...
		$App->item->active = 1;
		$App->pageSubTitle = preg_replace('/%ITEM%/',$localStrings['nazione'],$localStrings['inserisci %ITEM%']);
	...
	case 'insertNation':
		if ($_POST) {
			...
			Form::parsePostByFields($App->params->fields['nations'],$localStrings,array());
			...
			$_SESSION['message'] = '0|'.ucfirst(preg_replace('/%ITEM%/',$localStrings['nazione'],$localStrings['%ITEM% inserita'])).'!';
			ToolsStrings::redirect(URL_SITE.Core::$request->action.'/listNation');
	...
	case 'modifyNation':	
		...
		$App->pageSubTitle = preg_replace('/%ITEM%/',$localStrings['nazione'],$localStrings['modifica %ITEM%']);
	...
	case 'updateNation':
		if ($_POST) {
			...
			$_SESSION['message'] = '0|'.ucfirst(preg_replace('/%ITEM%/',$localStrings['nazione'],$localStrings['%ITEM% modificata'])).'!';
	...
	default;
		...
		Sql::setOrder('title_'.$localStrings['user'].' '.$App->params->orderTypes['nations']);	
		...
		$App->pageSubTitle = preg_replace('/%ITEMS%/',$localStrings['nazioni'],$localStrings['lista %ITEMS%']);
		$App->viewMethod = 'list';	
	break;	
}
