...
switch (Core::$request->method) {
	...
	case 'disactiveProvin':
		Sql::manageFieldActive(substr(Core::$request->method,0,-6),$App->params->tables['province'],$App->id,array('label'=>$localStrings['provincia'],'attivata'=>$localStrings['attivata'],'disattivata'=>$localStrings['disattivata']));
	...
	case 'deleteProvin':
		if ($App->id > 0) {
			...
			$_SESSION['message'] = '0|'.ucfirst(preg_replace('/%ITEM%/',$localStrings['provincia'],$localStrings['%ITEM% cancellata'])).'!';
			ToolsStrings::redirect(URL_SITE.Core::$request->action.'/listProvin');
	...
	case 'newProvin':
		...
		$App->item->active = 1;
		$App->pageSubTitle = preg_replace('/%ITEM%/',$localStrings['provincia'],$localStrings['inserisci %ITEM%']);
	...
	case 'insertProvin':
		if ($_POST) {
			...
			Form::parsePostByFields($App->params->fields['province'],$localStrings,array());
			...
			$_SESSION['message'] = '0|'.ucfirst(preg_replace('/%ITEM%/',$localStrings['provincia'],$localStrings['%ITEM% inserita'])).'!';
			ToolsStrings::redirect(URL_SITE.Core::$request->action.'/listProvin');
	...
	case 'modifyProvin':	
		...
		$App->pageSubTitle = preg_replace('/%ITEM%/',$localStrings['provincia'],$localStrings['modifica %ITEM%']);
	...
	case 'updateProvin':
		if ($_POST) {
			...
			$_SESSION['message'] = '0|'.ucfirst(preg_replace('/%ITEM%/',$localStrings['provincia'],$localStrings['%ITEM% modificata'])).'!';
	...
	default;
		...
		$App->pageSubTitle = preg_replace('/%ITEMS%/',$localStrings['province'],$localStrings['lista %ITEMS%']);
		$App->viewMethod = 'list';	
	break;	
}
