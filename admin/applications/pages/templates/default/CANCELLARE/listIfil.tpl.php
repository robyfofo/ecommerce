<!-- wscms/pages/listIfil.tpl.php v.3.5.4. 09/05/2019 -->
<div class="row">
	<div class="col-md-3 new">
 		<a href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/newIfil" title="{{ Lang['inserisci un nuovo file']|capitalize }}" class="btn btn-primary">{{ Lang['nuovo file']|capitalize }}</a>
	</div>
	<div class="col-md-7 help-small-list">
		{% if App.params.help_small is defined and App.params.help_small != '' %}{{ App.params.help_small }}{% endif %}
	</div>
	<div class="col-md-2">
		<a href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/listItem" title="{{ Lang['torna alla lista pagine']|capitalize }}" class="btn btn-primary">{{ Lang['lista pagine']|capitalize }}</a>
	</div>
</div>
...
<form role="form" action="{{ URLSITEADMIN }}{{ CoreRequest.action }}/listIfil" method="post" enctype="multipart/form-data">
	<div class="row">
		<div class="col-md-12">
								...
								{% for key,value in App.items %}
									<tr>
										...
										<td class="ordering">
											...
											{% endif %}
											<a class="" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/{{ App.params.ordersType['ifil'] == 'ASC' ? 'less' : 'more' }}OrderingIfil/{{ value.id }}" title="{{ Lang['sposta']|capitalize }} {{ App.params.ordersType['ifil'] == 'DESC' ? Lang['giu'] : Lang['su'] }}"><i class="fa fa-long-arrow-up"> </i></a>
											<a class="" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/{{ App.params.ordersType['ifil'] == 'ASC' ? 'more' : 'less' }}OrderingIfil/{{ value.id }}" title="{{ Lang['sposta']|capitalize }}  {{ App.params.ordersType['ifil'] == 'DESC' ? Lang['su'] : Lang['giu'] }}"><i class="fa fa-long-arrow-down"> </i></a>
										</td>								
										<td>{{ value.title }}</a></td>
										<td>	
											<a class="" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/downloadIfil/{{ value.id }}" title="{{ Lang['scarica il file'] }} {{ value.org_filename }}">
												{{ value.filename }}
											</a>
										</td>	
										<td class="actions">
											<a class="btn btn-default btn-circle" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/{{ value.active == 1 ? 'disactive' : 'active' }}Ifil/{{ value.id }}" title="{{ value.active == 1 ? Lang['disattiva']|capitalize : Lang['attiva']|capitalize }} {{ Lang['file'] }}"><i class="fa fa-{{ value.active == 1 ? 'unlock' : 'lock' }}"> </i></a>			 
											<a class="btn btn-default btn-circle" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/modifyIfil/{{ value.id }}" title="{{ Lang['modifica']|capitalize }} {{ Lang['file'] }}"><i class="fa fa-edit"> </i></a>
											<a class="btn btn-default btn-circle confirm" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/deleteIfil/{{ value.id }}" title="{{ Lang['cancella']|capitalize }} {{ Lang['file'] }}"><i class="fa fa-cut"> </i></a>
										</td>							
									</tr>	
								{% endfor %}
							{% else %}
						...
						<div class="dataTables_paginate paging_simple_numbers" id="dataTables_paginate">
							<ul class="pagination">
								<li class="paginate_button previous{% if App.pagination.page == 1 %} disabled{% endif %}">
									<a href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/pageIfil/{{ App.pagination.itemPrevious }}">{{ Lang['precedente']|capitalize }}</a>
								</li>									
								{% if App.pagination.pagePrevious is iterable %}
									{% for key,value in App.pagination.pagePrevious %}
										<li><a href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/pageIfil/{{ value }}">{{ value }}</a></li>
									{% endfor %}
								{% endif %}										
								<li class="active"><a href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/pageIfil/{{ App.pagination.page }}">{{ App.pagination.page }}</a></li>										
								...
								<li class="paginate_button next {% if App.pagination.page >= App.pagination.totalpage %} disabled{% endif %}">
									<a href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/pageIfil/{{ App.pagination.itemNext }}">{{ Lang['prossima']|capitalize }}</a>
								</li>
							</ul>
						</div>
					</div>
