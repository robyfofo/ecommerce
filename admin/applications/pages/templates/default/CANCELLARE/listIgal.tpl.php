<!-- wscms/pages/listIgal.tpl.php v.3.5.2. 19/02/2018 -->
<div class="row">
	<div class="col-md-3 new">
 		<a href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/newIgal" title="{{ Lang['inserisci una nuova immagine galleria']|capitalize }}" class="btn btn-primary">{{ Lang['nuova immagine']|capitalize }}</a>
	</div>
...
<form role="form" action="{{ URLSITEADMIN }}{{ CoreRequest.action }}/listIgal" method="post" enctype="multipart/form-data">
	<div class="row">
		<div class="col-md-12">
								...
								{% for key,value in App.items %}
									<tr>
										...
										<td class="ordering">
											...
											{% endif %}
											<a class="" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/{{ App.params.ordersType['iimg'] == 'ASC' ? 'less' : 'more' }}OrderingIgal/{{ value.id }}" title="{{ Lang['sposta']|capitalize }} {{ App.params.ordersType['iimg'] == 'DESC' ? Lang['giu'] : Lang['su'] }}"><i class="fa fa-long-arrow-up"> </i></a>
											<a class="" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/{{ App.params.ordersType['iimg'] == 'ASC' ? 'more' : 'less' }}OrderingIgal/{{ value.id }}" title="{{ Lang['sposta']|capitalize }}  {{ App.params.ordersType['iimg'] == 'DESC' ? Lang['su'] : Lang['giu'] }}"><i class="fa fa-long-arrow-down"> </i></a>
										</td>								
										<td>{{ value.title }}</a></td>
										<td>
											{% if value.filename != '' %}
										...
										<td class="actions">
											<a class="btn btn-default btn-circle" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/{{ value.active == 1 ? 'disactive' : 'active' }}Igal/{{ value.id }}" title="{{ value.active == 1 ? Lang['disattiva']|capitalize : Lang['attiva']|capitalize }} {{ Lang['immagine'] }}"><i class="fa fa-{{ value.active == 1 ? 'unlock' : 'lock' }}"> </i></a>			 
											<a class="btn btn-default btn-circle" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/modifyIgal/{{ value.id }}" title="{{ Lang['modifica']|capitalize }} {{ Lang['immagine'] }}"><i class="fa fa-edit"> </i></a>
											<a class="btn btn-default btn-circle confirm" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/deleteIgal/{{ value.id }}" title="{{ Lang['cancella']|capitalize }} {{ Lang['immagine'] }}"><i class="fa fa-cut"> </i></a>
										</td>							
									</tr>	
								{% endfor %}
							{% else %}
						...
						<div class="dataTables_paginate paging_simple_numbers" id="dataTables_paginate">
							<ul class="pagination">
								<li class="paginate_button previous{% if App.pagination.page == 1 %} disabled{% endif %}">
									<a href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/pageIgal/{{ App.pagination.itemPrevious }}">{{ Lang['precedente']|capitalize }}</a>
								</li>									
								{% if App.pagination.pagePrevious is iterable %}
									{% for key,value in App.pagination.pagePrevious %}
										<li><a href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/pageIgal/{{ value }}">{{ value }}</a></li>
									{% endfor %}
								{% endif %}										
								<li class="active"><a href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/pageIgal/{{ App.pagination.page }}">{{ App.pagination.page }}</a></li>										
								...
								<li class="paginate_button next {% if App.pagination.page >= App.pagination.totalpage %} disabled{% endif %}">
									<a href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/pageIgal/{{ App.pagination.itemNext }}">{{ Lang['prossima']|capitalize }}</a>
								</li>
							</ul>
						</div>
					</div>
