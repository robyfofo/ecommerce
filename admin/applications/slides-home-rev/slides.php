 ...
 * admin/slides-home-rev/slides.php v.1.0.0. 20/03/2021
*/
...
switch(Core::$request->method) {
	case 'moreOrderingItem':
		Utilities::increaseFieldOrdering($App->id,Config::$localStrings,array('table'=>$App->params->tables['item'],'orderingType'=>$App->params->ordersType['item'],'parent'=>1,'parentField'=>'section','label'=>ucfirst(Config::$localStrings['voce']).' '.Config::$localStrings['spostato']));
	...
	case 'lessOrderingItem':
		Utilities::decreaseFieldOrdering($App->id,Config::$localStrings,array('table'=>$App->params->tables['item'],'orderingType'=>$App->params->ordersType['item'],'parent'=>1,'parentField'=>'section','label'=>ucfirst(Config::$localStrings['voce']).' '.Config::$localStrings['spostato']));
		$_SESSION['message'] = Core::$resultOp->type.'|'.Core::$resultOp->message;
		ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listItem');
	break;
	...
	case 'deleteItem':
		...
		if ($App->id > 0) {
			...
			if ($count > 0) {
				$_SESSION['message'] = '2|'.ucfirst(preg_replace('/%ITEM%/',Config::$localStrings['layers'],Config::$localStrings['Ci sono ancora %ITEM% associati!']));
				ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listItem');
			}
...
switch((string)$App->viewMethod) {
	case 'form':
		$App->templateApp = 'formSlide.html';
		$App->jscript[] = '<script src="'.URL_SITE_ADMIN.$App->pathApplications.Core::$request->action.'/templates/'.$App->templateUser.'/js/formSlide.js"></script>';
	break;

	case 'list':
	default:
		$App->templateApp = 'listSlides.html';	
		$App->jscript[] = '<script src="'.URL_SITE_ADMIN.$App->pathApplications.Core::$request->action.'/templates/'.$App->templateUser.'/js/listSlides.js"></script>';	
	break;
}	
?>