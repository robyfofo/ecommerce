...
switch(Core::$request->method) {
	case 'updateConf':
		...
		Form::parsePostByFields($App->params->fields['conf'],$localStrings,array());
		if (Core::$resultOp->error > 0) { 
			$_SESSION['message'] = '1|'.implode('<br>', Core::$resultOp->messages);
		...
		$_SESSION['message'] = '0|'.ucfirst(preg_replace('/%ITEM%/',$localStrings['configurazione'],$localStrings['%ITEM% modificata']));
		ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/modifyConf/');
										
	break;
	
	default;	
		$App->pageSubTitle = $localStrings['configurazione'];
		$App->viewMethod = 'form';	
	break;	
	}
