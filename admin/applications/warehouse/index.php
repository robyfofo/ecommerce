 ...
 * admin/warehouse/index.php v.1.0.0. 24/02/2021
*/
...
switch($moduleRedirect) {
	...
	case 'Proa':	
		$App->sessionName = $App->sessionName.'-products-attributes';
		if (!isset($_SESSION[$App->sessionName]['products_attributes_types_id'])) $_SESSION[$App->sessionName]['products_attributes_types_id'] = 0;
	...
	case 'Prat':	
		$App->sessionName = $App->sessionName.'-products-attributes-types';	
		if (!isset($_MY_SESSION_VARS[$App->sessionName]['page'])) $_MY_SESSION_VARS = $my_session->addSessionsModuleVars($_MY_SESSION_VARS,$App->sessionName,array('page'=>1,'ifp'=>'10','srcTab'=>''));
		$Module = new Module($App->sessionName,$App->params->tables['proatypes']);		
		include_once(PATH.$App->pathApplications.Core::$request->action."/product-attributes-types.php");	
	break;
