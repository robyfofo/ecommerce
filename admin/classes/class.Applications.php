 ...
 * classes/class.Applications.php v.1.0.0. 16/02/2021
*/
...
class Applications extends Core 
{
	...
	public static function setDataTableArrayVars($array,$request,$userLoggedData,$params) {
		...
		/* permissions query */
		list($permClause,$fieldsValuesPermClause) = Permissions::getSqlQueryItemPermissionForUser($userLoggedData,array('fieldprefix'=>'ite.','onlyuser'=>true));
		...
		if (isset($request['search']) && is_array($request['search']) && count($request['search']) > 0) {		
			if (isset($request['search']['value']) && $request['search']['value'] != '') {
				list($w,$fv) = Sql::getClauseVarsFromAppSession($request['search']['value'],$params->fields['item'],'',array('tableAlias'=>'ite'));
				if ($w != '') {
					$wf[] = $w;
					$wfv = $fv;
				}		
				...
				if (is_array($array['whereFields']) && count($array['whereFields']) > 0) {	
					...
					foreach ($array['whereFields'] AS $keyqw=>$valueqw) {						
						switch($valueqw['type']) {	
							...
							case 'datalabelint':
								$keys = preg_grep( '/'.$request['search']['value'].'/', $App->params->status );
