	...
	public static function validateField($fieldName, $fieldDetails, $fieldPostValueArray,$fieldLabel,$fieldPostValue)
	{
		...
		switch ($fieldDetails['validate']) {
			...
			case 'datetimeiso':
				...
				if ($result == false) {
					$message = preg_replace('/%FIELD%/',$fieldLabel,Config::$localStrings['La data %FIELD% non è valida!']);
				}
			break;
			case 'minmax':
			...
			case 'time':
				...
				if ($result == false) {
					$s = Config::$localStrings['Il campo %FIELD% non è un tempo valido!'];
					$message = preg_replace('/%FIELD%/', $fieldLabel, $s);
				}
			break;
			case 'explodearray':
			...
			case 'currency':
				...
				if ($result == false) {
					$message = preg_replace('/%FIELD%/',$fieldLabel,Config::$localStrings['Il valore %FIELD% non è di un formato valuta!']);	
				}
			break;
			default:
				$returnvalue = '';
