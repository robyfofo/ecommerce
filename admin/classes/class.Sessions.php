<?php
/* wscms/classes/class.Sessions.php v.3.5.3. 26/07/2018 */

class my_session extends Core {
   ...
   public function my_session_register($name, $value) {   
      $_MY_SESSION = array();
      $sql = "SELECT SQL_NO_CACHE session_vars FROM ".$this->table_name." WHERE sessid = '{$this->my_session_id}'";
      $pdoCore = Sql::getInstanceDb();
      $result = $pdoCore->query($sql) or die('Errore db linea 48!');
      if ($pdoCore->query("SELECT SQL_NO_CACHE FOUND_ROWS()")->fetchColumn() > 0) {
   ...
   public function my_session_gc() {
  	...
  	public function addSessionsModuleVars($sessionsVars,$app,$sessions) {
		$refresh = false;
  	...
  	public function addSessionsSingleVar($sessionsVars,$key,$value) {
		$this->my_session_register($key,$value);	
		$sessionsVars = array();
		$sessionsVars = $this->my_session_read();
  		return $sessionsVars;  			
  		}	
  		
  	public function addSessionsModuleSingleVar($sessionsVars,$module,$key,$value) {
  		$sessionsVars[$module][$key] = $value;
		$this->my_session_register($module,$sessionsVars[$module]);	
		$sessionsVars = array();
		$sessionsVars = $this->my_session_read();
  		return $sessionsVars;  			
  		}	
	...
	public function rewriteSessionCookie() {
		 $cookie_expire = ($this->session_time > 0) ? (time() + $this->session_time) : 0;
		  if(isset($_COOKIE[$this->cookie_name])) {
